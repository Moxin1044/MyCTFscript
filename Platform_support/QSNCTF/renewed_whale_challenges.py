...
def renewed_whale_challenges(challenges_id, cookies):
    ...
    response = requests.patch(burp0_url, headers=burp0_headers, cookies=burp0_cookies)
    if response.json()['message'] == "Container Renewed":
        return "容器续期成功"
    return "请等待15秒后再试"  # 如果正常，结果只有成功一种。


print(renewed_whale_challenges(215, get_qsnctf_session()))
