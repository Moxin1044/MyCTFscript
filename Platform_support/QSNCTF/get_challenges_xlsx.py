...
def get_challenge_info(challenge_id, cookies):
    ...
    burp0_url = f"https://www.qsnctf.com:443/api/v1/challenges/{challenge_id}"
    burp0_headers = {"Sec-Ch-Ua": "\"Chromium\";v=\"109\", \"Not_A Brand\";v=\"99\"", "Accept": "application/json",
                     ...
                     "Sec-Fetch-Dest": "empty", "Referer": "https://www.qsnctf.com/challenges",
                     "Accept-Encoding": "gzip, deflate", "Accept-Language": "zh-CN,zh;q=0.9"}
    ...
    if response['success']:
        ...
        new_files = []
        for file in files:
            new_files.append(url + file)
        return_response = {"description": description, "files": new_files}
...
def format_challenges(cookies):
    ...
    response = requests.get(url, headers=burp0_headers, cookies=cookies).json()

    if response['success']:
        challenge_list = []
        for challenge in response['data']:
            dict1 = {"id": challenge['id'], "type": challenge['type'], "name": challenge['name'], "value": challenge['value'], "solves": challenge['solves'], "category": challenge['category']}
            dict2 = get_challenge_info(challenge['id'], cookies)
            dict1.update(dict2)
            print(dict1)
            challenge_list.append(dict1)
        # 将列表转换为DataFrame
        df = pd.DataFrame(challenge_list)
        # 将DataFrame写入Excel文件
